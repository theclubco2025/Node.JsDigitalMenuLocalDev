...
import { smsNotificationsEnabled } from '@/lib/notifications/twilio'

export async function runRetentionCron(now: Date = new Date()) {
  const sms = smsNotificationsEnabled()
    ? await runRetentionSmsCron(now)
    : { ok: true, skipped: 'sms_not_enabled', scanned: 0, queued: 0, sent: 0, failed: 0 }
