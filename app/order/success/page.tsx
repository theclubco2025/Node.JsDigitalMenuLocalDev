      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-white/70">Thanks for ordering from {tenantName}</p>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5">
          <div className="flex items-center justify-between gap-3">
            <div className="text-sm font-extrabold text-white">Order Status</div>
            {pickupCode && (
              <div className="text-right">
                <div className="text-[11px] font-semibold uppercase tracking-wide text-white/50">Code</div>
                <div className="font-mono text-lg font-extrabold text-white">{pickupCode}</div>
                {!isDineIn && (
                  <div className="mt-2 flex items-center justify-end gap-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center rounded-lg border border-white/15 bg-white/5 px-2.5 py-1 text-xs font-bold text-white hover:bg-white/10"
                    >
          ...
          <div className="mt-4 space-y-3">
            <div className="flex items-center gap-3">
              <div className={`h-3 w-3 rounded-full ${steps.received ? 'bg-white' : 'bg-white/25'}`} />
              <div className="text-sm font-semibold text-white">Received</div>
            </div>
            <div className="flex items-center gap-3">
              <div className={`h-3 w-3 rounded-full ${steps.preparing ? 'bg-white' : 'bg-white/25'}`} />
            ...
            <div className={`flex items-center gap-3 ${steps.ready ? 'animate-pulse' : ''}`}>
              <div className={`h-3 w-3 rounded-full ${steps.ready ? 'bg-emerald-400' : 'bg-white/25'}`} />
              <div className={`text-sm font-extrabold ${steps.ready ? 'text-emerald-200' : 'text-white'}`}>{step3Label}</div>
            </div>
          </div>
          ...
          {order && (
            <div className="mt-4 grid grid-cols-1 gap-2 text-sm text-white/80">
              ...
              {steps.ready && !isCanceled(order.status) && (
                <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 font-semibold text-emerald-100">
                  {readyMessage}
                </div>
              )}
            </div>
        ...
        {order && (
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5">
            <div className="text-sm font-extrabold text-white">Total Paid</div>
